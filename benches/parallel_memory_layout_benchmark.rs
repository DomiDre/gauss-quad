...
fn bench_degrees(c: &mut Criterion, id: &str, degs: &[usize]) {
    {
        ...
        for deg in degs {
            ...
            group.bench_function(format!("unified, degree {deg}"), |b| {
                b.iter(|| core::hint::black_box(too_rule_for_school.integrate(0.0, 2.0, |x| x * x)))
            });
        }
    }
    {
        ...
        for deg in degs {
            ...
            group.bench_function(format!("split, degree {deg}"), |b| {
                b.iter(|| core::hint::black_box(rulest_of_rules.par_integrate(0.0, 2.0, |x| x * x)))
            });
            let i_fought_the_rule_and_the_rule_won = GaussLegendreUnified::new(*deg);
            group.bench_function(format!("unified, degree {deg}"), |b| {
                b.iter(|| {
                    core::hint::black_box(i_fought_the_rule_and_the_rule_won.par_integrate(
                        0.0,
                        2.0,
                        |x| x * x,
                    ))
...
fn benches(c: &mut Criterion) {
    ...
    bench_degrees(c, "small x^2", &DEGS);
    bench_degrees(c, "big x^2", &BIG_DEGS);
}
