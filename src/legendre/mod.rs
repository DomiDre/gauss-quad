...
impl GaussLegendre {
        ...
        F: Fn(f64) -> f64,
    {
        let result: f64 = self
            .node_weight_pairs
            .iter()
            .map(|(node, weight)| integrand(Self::argument_transformation(node.0, a, b)) * weight.0)
            .sum();
...
mod tests {
    ...
    fn check_degree_3() {
        ...
        for (i, x_val) in x_should.iter().enumerate() {
            assert_abs_diff_eq!(*x_val, x[i].0);
        }
        for (i, w_val) in w_should.iter().enumerate() {
            assert_abs_diff_eq!(*w_val, w[i].0);
        }
    }
    ...
    fn check_degree_128() {
        ...
        for (i, x_val) in x_should.iter().rev().enumerate() {
            assert_abs_diff_eq!(*x_val, x[i].0, epsilon = 0.000_000_1);
        }
        for (i, w_val) in w_should.iter().rev().enumerate() {
            assert_abs_diff_eq!(*w_val, w[i].0, epsilon = 0.000_000_1);
        }
    }
